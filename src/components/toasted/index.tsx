import { title } from "process";
import { FC } from "react";

export const Toast: FC<{
  status: boolean;
  title: string;
  description: string;
  handleClose: any;
}> = ({ handleClose, title, description, status }) => {
  return (
    <div
      id="toast-default"
      className={`absolute bottom-10 right-10 flex items-start   p-4 text-gray-500 rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 ${
        status ? "bg-button-active" : "bg-error"
      }`}
      role="alert"
    >
      <div className=" text-sm font-normal w-[432px]">
        <p className="text-[20px]">{title}</p>
        <p className="text-[12px] pt-4">{description}</p>
      </div>
      <button
        onClick={handleClose}
        type="button"
        className="ml-auto -mx-1.5 -my-1.5  text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700"
        data-dismiss-target="#toast-default"
        aria-label="Close"
      >
        <span className="sr-only">Close</span>
        <svg
          className="w-3 h-3"
          aria-hidden="true"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 14 14"
        >
          <path
            stroke="currentColor"
            stroke-linecap="round"
            stroke-linejoin="round"
            stroke-width="2"
            d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"
          />
        </svg>
      </button>
    </div>
  );
};
